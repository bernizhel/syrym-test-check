...
function createRootReducer(state: Partial<ApplicationState>) {
  ...
  const fn = (state: ApplicationState = defaultState, action: Action): ApplicationState =>
    produce(state, (draft) => {
      switch (action.type) {
        case "PRODUCTS_LOAD":
...
// epics
const productsLoadEpic: ExampleEpic = (action$, store$, { api }) =>
  action$.pipe(
    ofType("PRODUCTS_LOAD"),
    mergeMap((a: ReturnType<typeof productsLoad>) => {
      return from(api.getProducts()).pipe(map((products) => productsLoaded(products.data)));
    })
  );

const productDetailsLoadEpic: ExampleEpic = (action$, store$, { api }) =>
  action$.pipe(
    ofType("PRODUCT_DETAILS_LOAD"),
    mergeMap((a: ReturnType<typeof productDetailsLoad>) => {
      return from(api.getProductById(a.id)).pipe(map((products) => productDetailsLoaded(products.data)));
    })
  );

const shoppingCartEpic: ExampleEpic = (action$, store$, { cart }) =>
  action$.pipe(
...
const checkoutEpic: ExampleEpic = (action$, store$, { api }) =>
  action$.pipe(
    ofType("CHECKOUT"),
    mergeMap(({ form, cart }: ReturnType<typeof checkout>) => {
      ...
      return from(api.checkout(form, cart)).pipe(map((res) => checkoutComplete(res.data.id)));
    })
  );

export const rootEpic = combineEpics(checkoutEpic, shoppingCartEpic, productsLoadEpic, productDetailsLoadEpic);

export function initStore(api: ExampleApi | any, cart: CartApi | any) {
